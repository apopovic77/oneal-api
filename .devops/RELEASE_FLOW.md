...
6. Validate the site at `https://oneal-api.arkturian.com` (if applicable) and confirm fresh assets reached `/var/www/oneal-api`.

## Rollback Strategy
- Use `.devops/rollback.sh` on the server to restore from `/var/backups/oneal-api-*`.
