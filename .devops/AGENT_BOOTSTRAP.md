...
1. **Repository context**
   - Project lives at `/Volumes/DatenAP/Code/oneal-api`.
...
4. **Deployment target**
   - Built files go to `/var/www/oneal-api`.
   - Backups are stored with prefix `oneal-api` under `/var/backups/` (configurable via templates).
